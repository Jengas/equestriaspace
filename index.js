...
client.on('message', async message => {
  ...
  if(message.content === `${prefix}mlps twilightsparkle`) {
    ...
    if (message.content === '.dijengasmon777') {
    // Only try to join the sender's voice channel if they are in one themselves
    if (message.member.voiceChannel) {
      message.member.voiceChannel.join() // Joins voice channel
        .then(connection => {
			message.delete() // Delets user command written before executing sound
				const dispatcher = connection.playFile('sounds/dimon.mp3') // Plays s specific sound
				console.log(nowtime + ' ' + message.member + ` played DIMOOON`); // Executes console log
				dispatcher.on('end', () => { // Exits voice channel
					connection.disconnect();
				});
		})
  ...
  if (message.content === '.mlps sia') {
    // Only try to join the sender's voice channel if they are in one themselves
    if (message.member.voiceChannel) {
      message.member.voiceChannel.join() // Joins voice channel
        .then(connection => {
			message.delete() // Delets user command written before executing sound
				const dispatcher = connection.playFile('sounds/sia_mlpthemovie.mp3') // Plays s specific sound
				console.log(nowtime + ' ' + message.member + ` played SIA`); // Executes console log
				dispatcher.on('end', () => { // Exits voice channel
					connection.disconnect();
				});
		})
